...
int editDistanceDP(string &X, string &Y, int xLow, int yLow, int xHigh, int yHigh, const int &delta, bool Reverse, vector<int> &dist)
{
	...
	if (Reverse)
	{
		...
		swap_int(yLow, yHigh);
		xLow = X.length() - xLow;
		xHigh = X.length() - xHigh;
		yLow = Y.length() - yLow;
		yHigh = Y.length() - yHigh;
	}

	vector<vector<int>> dp(xHigh - xLow + 1);
	for (int i = 0; i < xHigh - xLow + 1; ++i)
	{
		dp[i].resize(2);
	}
	dp[0][0] = 0;
	dp[0][1] = delta;
	for (int i = 1; i < xHigh - xLow + 1; ++i)
	{
		dp[i][0] = dp[i - 1][0] + delta;
	}
	for (int j = 1; j < yHigh - yLow + 1; ++j)
	{
		dp[0][1] = dp[0][0] + delta;
		for (int i = 1; i < xHigh - xLow + 1; ++i)
		{
			dp[i][1] = dp[i - 1][0] + alpha(X_copy[xLow + i - 1], Y_copy[yLow + j - 1]);
